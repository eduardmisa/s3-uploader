      ...
      const results = await Promise.all(
        items.map(async (item) => {
          try {
            ...
            if (!thumbnailUrl) {
              return { url: item.url, status: "failed", error: "Could not generate thumbnail URL" };
            }
            const thumbnailBlob = await generateThumbnail(item.url);
            if (thumbnailBlob) {
            ...
            return { url: item.url, status: "failed", error: "Thumbnail generation failed" };
          } catch (error: any) {
