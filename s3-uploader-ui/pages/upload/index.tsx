...
import { ChevronLeft, ChevronRight, FolderIcon, FileImageIcon, HomeIcon, PlusCircleIcon, UploadIcon } from 'lucide-react';
...
export default function UploadPage() {
  ...
  const findNodeInTree = (nodes: TreeNode[], name: string): TreeNode | undefined => {
  ...
  const handleAction = (key: React.Key, newHistory?: string[]) => {
    ...
    if (node) {
      if (node.type === 'file' && node.url) {
        ...
        if (!newHistory)
          setPathHistory(prev => [...prev, node.name]);
        else
    ...
    return (
      <>
        {currentFolder.map(node => (
          <ListboxItem
            key={node.name}
            startContent={node.type === 'folder' ? <FolderIcon /> : <FileImageIcon />}
      ...
      <CardFooter className="flex flex-col text-white/60 gap-4">
        <div className="flex flex-col w-full grow gap-2 items-center">
          {fileList.map((fileList, index) => (
            <div className="flex w-full items-center gap-2" key={`${fileList.id}-${index}`}>
              <Image
              ...
              <div className="flex flex-col w-full gap-2">
                ...
                <Progress aria-label="Loading..." size="sm" className="max-w-md" value={fileList.progress} color={fileList.progress === 100 ? "success" : "primary"} />
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="flex flex-row items-start justify-center gap-4 py-8 md:py-10">
        ...
        >
          <ListboxItem
            ...
            key={"ROOT"}
            startContent={
              <Button size="lg" isIconOnly variant="flat" onPress={handleBack}>
                <ChevronLeft />
              </Button>
            }
            endContent={
              <Button size="lg" isIconOnly variant="flat" onPress={() => handleAction("ROOT")}>
                <HomeIcon />
              </Button>
            }
            showDivider
          ...
          >
            Create folder
            <PlusCircleIcon size={17} className="inline mb-1 ml-1" />
          </ListboxItem>
        </Listbox>

        <Card className="py-4">
          <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
            <p className="text-tiny uppercase font-bold">To upload <small className="text-default-500">{queued.length || 0} Files</small></p>

            <h4 className="font-bold text-large">Uploading to <span className="underline">{pathHistory[pathHistory.length - 1] || "ROOT"}</span> folder</h4>
          </CardHeader>
          <CardBody className="overflow-visible py-2">
            <Dropzone onFilesDropped={handleFilesDropped} />

            <Button isLoading={activeUploadsCount > 0} color={canUpload ? "success" : "default"} variant={canUpload ? "shadow" : "faded"} onPress={startUploads} disabled={!canUpload}>
              {canUpload ? "Start Upload" : "Select/Drop files to upload" }  <UploadIcon size={15} />
            </Button>
          </CardBody>

          {queued && queued.length > 0 && renderUploadStatusCardFooter(queued)}
          {uploading && uploading.length > 0 && renderUploadStatusCardFooter(uploading)}
          {uploaded && uploaded.length > 0 && renderUploadStatusCardFooter(uploaded)}
          ...
          {(onClose) => (
            <>
              ...
              <ModalFooter>
                ...
                <Button color="primary" onPress={() => onCreateFolder(newFolderName)}>
                  Save
                </Button>
              </ModalFooter>
            </>
...
const convertUrlsToTree = (urls: string[]): TreeNode[] => {
  ...
  urls.forEach(url => {
    ...
    pathParts.forEach((part, index) => {
      let existingNode = currentLevel.find(node => node.name === part && (index === pathParts.length - 1 ? node.type === 'file' : node.type === 'folder'));
